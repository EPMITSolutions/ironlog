...
import { dbGetAll, dbPut, dbDelete, dbGetByIndex, uuid, initDB } from '@/lib/db'
import { calc1RM } from '@/lib/utils'

export const useProfile = create<any>((set, get) => ({
  profile: null, initialized: false,
  init: async () => {
    await initDB()
  ...
  save: async (fullName: string, goal: string) => {
    ...
    await dbPut('profile', updated); set({ profile: updated })
  },
...
export const useRoutines = create<any>((set, get) => ({
  routines: [], active: null, loading: false,
  load: async () => {
    set({ loading: true })
    const [routines, days, res, exercises] = await Promise.all([dbGetAll('routines'), dbGetAll('routine_days'), dbGetAll('routine_exercises'), dbGetAll('exercises')])
  ...
  remove: async (id: string) => {
    ...
    for (const d of days) { const res = await dbGetByIndex('routine_exercises', 'routine_day_id', d.id); for (const re of res) await dbDelete('routine_exercises', re.id); await dbDelete('routine_days', d.id) }
    await dbDelete('routines', id); await get().load()
  },
  ...
  removeDay: async (dayId: string) => {
    ...
    await dbDelete('routine_days', dayId); await get().load()
  },
  addExercise: async (dayId: string, exerciseId: string, config: any) => {
    ...
    await dbPut('routine_exercises', { id: uuid(), routine_day_id: dayId, exercise_id: exerciseId, order_index: existing.length, target_sets: config.sets ?? 3, target_reps: config.reps ?? 8, target_reps_max: config.repsMax ?? 12, rest_seconds: config.rest ?? 90, target_weight: null, notes: null })
    await get().load()
  },
  removeExercise: async (id: string) => { await dbDelete('routine_exercises', id); await get().load() },
}))

export const useWorkout = create<any>()(
  persist(
    (set, get) => ({
      session: null, saving: false,
      ...
      addExtra: (exId: string) => {
        const s = get().session; if (!s) return
        const prev = s.sets[exId] ?? []; const last = prev[prev.length - 1]
      ...
      tick: () => {
        const s = get().session; if (!s?.rest.active) return
      ...
      finish: async (notes: string, rpe: number) => {
        ...
        for (const [exId, sets] of Object.entries(s.sets) as [string, any[]][]) {
          for (let i = 0; i < sets.length; i++) {
            const set = sets[i]; if (!set.done || !set.weight || !set.reps) continue
            const orm = calc1RM(set.weight, set.reps); let isPR = false
            if (!set.isWarmup) {
              const cur = prs.find((p: any) => p.exercise_id === exId && p.pr_type === 'max_weight')
              if (!cur || set.weight > cur.value) { isPR = true; await dbPut('personal_records', { id: cur?.id ?? uuid(), exercise_id: exId, pr_type: 'max_weight', value: set.weight, achieved_at: finishedAt }) }
              const cur1rm = prs.find((p: any) => p.exercise_id === exId && p.pr_type === 'max_1rm')
              if (!cur1rm || orm > cur1rm.value) await dbPut('personal_records', { id: cur1rm?.id ?? uuid(), exercise_id: exId, pr_type: 'max_1rm', value: orm, achieved_at: finishedAt })
            }
        ...
        set({ session: null, saving: false }); return sessionId
      },

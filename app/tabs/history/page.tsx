...
export default function History() {
  ...
  useEffect(() => {
    const load = async () => {
      const [allS, allSets, exs] = await Promise.all([dbGetAll('sessions'), dbGetAll('sets'), dbGetAll('exercises')])
      const exMap = Object.fromEntries(exs.map((e: any) => [e.id, e]))
      const done = allS.filter((s: any) => s.finished_at).sort((a: any, b: any) => b.started_at.localeCompare(a.started_at))
      ...
      for (const s of done) {
        ...
        m[s.id] = { volume: calcVolume(sets.map((x: any) => ({ weight_kg: x.weight_kg, reps: x.reps }))), muscles: muscles.slice(0, 4), prs: sets.filter((x: any) => x.is_pr).length }
      }
      setSessions(done); setMeta(m)
    }
    load()
  }, [])
  ...
  const byEx = dSets.reduce<Record<string, any[]>>((acc, s) => { if (!acc[s.exercise_id]) acc[s.exercise_id] = []; acc[s.exercise_id].push(s); return acc }, {})
  ...
  return (
    <div className="px-5 pt-14 pb-4">
      ...
      {sessions.length === 0 ? (
        <div className="text-center py-16 space-y-2"><p className="text-4xl">📅</p><p className="text-white font-medium">Sin historial todavía</p></div>
      ) : Object.entries(grouped).map(([month, list]) => (
        <div key={month}>
          <p className="font-heading text-sm text-white/30 tracking-widest uppercase mt-5 mb-2">{month}</p>
          {list.map((s: any) => {
            const d = new Date(s.started_at); const sm = meta[s.id]
            return (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-2 mt-0.5 flex-wrap">
                      ...
                      {sm?.volume > 0 && <span className="text-white/30 text-xs">📦 {Math.round(sm.volume)}kg</span>}
                      {sm?.prs > 0 && <span className="text-red-400 text-xs font-semibold">🔥 {sm.prs} PR{sm.prs>1?'s':''}</span>}
                    </div>
                    {sm?.muscles?.length > 0 && (
                      <div className="flex gap-1 mt-1.5 flex-wrap">
                        {sm.muscles.map((m: string) => <span key={m} className="text-[10px] px-1.5 py-0.5 rounded-md font-medium" style={{backgroundColor:MUSCLE_COLORS[m]+'22',color:MUSCLE_COLORS[m]}}>{MUSCLE_LABELS[m]}</span>)}
                      </div>
                    )}
                  </div>
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-end" onClick={() => setDetail(null)}>
          <div className="w-full max-w-md mx-auto bg-surface rounded-t-3xl p-5 pb-10 max-h-[88vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <div><p className="font-heading text-2xl text-white tracking-wide">{detail.name ?? 'Entrenamiento'}</p><p className="text-white/30 text-sm capitalize">{new Date(detail.started_at).toLocaleDateString('es',{weekday:'long',day:'numeric',month:'long'})}</p></div>
            ...
            <div className="flex gap-4 flex-wrap mb-4 pb-4 border-b border-border">
              ...
              <div className="text-center"><p className="font-heading text-xl text-accent">{Math.round(calcVolume(dSets.map(s=>({weight_kg:s.weight_kg,reps:s.reps}))))}kg</p><p className="text-white/20 text-xs">Volumen</p></div>
              {dSets.filter(s=>s.is_pr).length>0 && <div className="text-center"><p className="font-heading text-xl text-red-400">{dSets.filter(s=>s.is_pr).length}🔥</p><p className="text-white/20 text-xs">PRs</p></div>}
            ...
            {detail.notes && <div className="bg-card border border-border rounded-xl p-3 mb-4 text-white/50 text-sm italic">"{detail.notes}"</div>}
            <div className="overflow-y-auto no-scroll space-y-4">
              {Object.entries(byEx).map(([exId, sets]) => (
                <div key={exId}>
                  <p className="text-white font-semibold text-sm mb-1.5">{(sets as any[])[0].exercise?.name_es}</p>
                  {(sets as any[]).map((s,i) => (

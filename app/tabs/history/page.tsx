...
import { fmtDuration, calcVolume } from '@/lib/utils'
...
export default function History() {
  ...
  useEffect(() => {
    dbGetAll('sessions').then(d => setSessions(d.filter((s: any) => s.finished_at).sort((a: any, b: any) => b.started_at.localeCompare(a.started_at))))
  }, [])
    ...
    <div className="px-5 pt-14 pb-4">
      ...
      {sessions.length === 0 ? (
        ...
        <div key={month}>
          ...
          {list.map((s: any) => {
            ...
            return (
              <button key={s.id} onClick={() => open(s)} className="w-full bg-card border border-border rounded-2xl p-3.5 mb-2 flex items-center gap-3 text-left">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-2 mt-0.5">
                    ...
                    {s.rpe!=null && <span className="text-white/30 text-xs">RPE {s.rpe}</span>}
                  </div>
                </div>
      ...
      {detail && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-end" onClick={() => setDetail(null)}>
          <div className="w-full max-w-md mx-auto bg-surface rounded-t-3xl p-5 pb-10 max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="font-heading text-2xl text-white tracking-wide">{detail.name ?? 'Entrenamiento'}</p>
                <p className="text-white/30 text-sm capitalize">{new Date(detail.started_at).toLocaleDateString('es',{weekday:'long',day:'numeric',month:'long'})}</p>
              </div>
            ...
            <div className="overflow-y-auto no-scroll space-y-4">
              {Object.entries(byEx).map(([exId, sets]) => (
                <div key={exId}>
                  <p className="text-white font-semibold text-sm mb-2">{(sets as any[])[0].exercise?.name_es ?? 'Ejercicio'}</p>
                  {(sets as any[]).map((s, i) => (
                    <div key={s.id} className="flex items-center gap-2 py-1.5">
                      <span className="text-accent text-xs font-bold w-6">S{i+1}</span>
